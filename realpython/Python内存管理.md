...
Arenas是最大内存块并且与内存里的页边界对齐。一个页边界操作系统使用的一个固定长度连续的内存块的边缘。Python假设系统的系统页面大小是256 kb。
...
还有，一个`已满池`会释放一些自己的内存块，因为这些内存块已不再被使用。然后这个内存池会根据其尺寸级会被重新添加到`已使用池（usedpools）`列表中。
...
从上面的图表可以看到，内存池包含一个指针指向它们的“空闲”内存块。这个的工作方式有些细微的差别。根据源码的注释中指出：这个分配器 “努力不去触碰所有层级（arena、内存池、内存块）的任何一块内存直到需要真的被需要”。
...
Arenas却是被组织成一个双链表叫做`usable_arenas`。里面的arenas是根据其空闲池的数量排序的。空闲池越少的arena越靠前。
